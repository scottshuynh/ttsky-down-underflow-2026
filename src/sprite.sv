...
module sprite #(
    ...
    always_comb begin
        case (state)
            IDLE:       state_next = state_t'(start ? START : IDLE);
            START:      state_next = state_t'(AWAIT_DMA);
            AWAIT_DMA:  state_next = state_t'(dma_avail ? READ_MEM : AWAIT_DMA);
            READ_MEM:   state_next = state_t'(AWAIT_POS);
            AWAIT_POS:  state_next = state_t'((sx == sprx-2) ? DRAW : AWAIT_POS);
            DRAW:       state_next = state_t'(!last_pixel ? DRAW :
                                    (!last_line ? NEXT_LINE : DONE));
            NEXT_LINE:  state_next = state_t'(load_line ? AWAIT_DMA : AWAIT_POS);
            DONE:       state_next = state_t'(IDLE);
            default:    state_next = state_t'(IDLE);
        endcase
    end
endmodule
